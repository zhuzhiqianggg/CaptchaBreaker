...
def test_image_accuracy(image_path, language="en", use_preprocessing=True):
    url = f"{BASE_URL}/ocr/upload"
    
    with open(image_path, "rb") as f:
        files = {"file": (os.path.basename(image_path), f, "image/png")}
        data = {"language": language, "use_preprocessing": str(use_preprocessing)}
...
def main():
    ...
    with open("ocr_test_results_optimized.json", "w", encoding="utf-8") as f:
        json.dump(results, f, indent=2, ensure_ascii=False)
    print(f"\n详细结果已保存到: ocr_test_results_optimized.json")
